...
class conv_norm_relu(nn.Module):
    '''
    Block of convolution in CloudSegNet
    '''
...
class CloudSegNet(nn.Module):
    def __init__(self):
        ...
        self.dec_conv1 = conv_norm_relu(8, 16)

        self.upsample2 = nn.MaxUnpool2d(2, 2)
        self.dec_conv2 = conv_norm_relu(16, 16)

        self.dec3 = conv_norm_relu(16, 1)
...
def train(model, opt, loss_fn, epochs, data_tr, device):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, Y_batch in tqdm(data_tr):
            ...
            loss = loss_fn(Y_batch, Y_pred.squeeze(1))
            loss.backward()
            opt.step()
